...
namespace arx
{
    ...
    private:
        ...
        void resolve_overflow()
        {
            if (empty())
                clear();
            else if (head_ <= (static_cast<int>(INT_MIN) + static_cast<int>(capacity())) || tail_ >= (static_cast<int>(INT_MAX) - static_cast<int>(capacity())))
            {
    ...
    template <typename T, size_t N, typename CMP>
    bool operator==(const RingBuffer<T, N, CMP> &x, const RingBuffer<T, N, CMP> &y)
    {
        if (x.size() != y.size())
            return false;
    ...
    template <typename T, size_t N = ARX_VECTOR_DEFAULT_SIZE, typename CMP = void *>
    struct vector : public RingBuffer<T, N, CMP>
    {
        using iterator = typename RingBuffer<T, N, CMP>::iterator;
        using const_iterator = typename RingBuffer<T, N, CMP>::const_iterator;

        vector()
            : RingBuffer<T, N, CMP>() {}
        vector(std::initializer_list<T> lst)
            : RingBuffer<T, N, CMP>(lst) {}

        // copy
        vector(const vector &r)
            : RingBuffer<T, N, CMP>(r) {}

        vector &operator=(const vector &r)
        {
            RingBuffer<T, N, CMP>::operator=(r);
            return *this;
        }
        ...
        using RingBuffer<T, N, CMP>::pop_front;

    private:
        using RingBuffer<T, N, CMP>::pop;
        using RingBuffer<T, N, CMP>::push;
        using RingBuffer<T, N, CMP>::push_front;
        using RingBuffer<T, N, CMP>::emplace;
        using RingBuffer<T, N, CMP>::fill;
    };

    template <typename T, size_t N, typename CMP>
    struct array : public RingBuffer<T, N, CMP>
    {
        using iterator = typename RingBuffer<T, N, CMP>::iterator;
        using const_iterator = typename RingBuffer<T, N, CMP>::const_iterator;
    ...
    template <typename T, size_t N = ARX_DEQUE_DEFAULT_SIZE, typename CMP = void *>
    struct deque : public RingBuffer<T, N, CMP>
    {
        using iterator = typename RingBuffer<T, N, CMP>::iterator;
        using const_iterator = typename RingBuffer<T, N, CMP>::const_iterator;
    ...
    private:
        using RingBuffer<T, N, CMP>::capacity;
        using RingBuffer<T, N, CMP>::pop;
        using RingBuffer<T, N, CMP>::push;
        using RingBuffer<T, N, CMP>::emplace;
        using RingBuffer<T, N, CMP>::assign;
        using RingBuffer<T, N, CMP>::begin;
        using RingBuffer<T, N, CMP>::end;
        using RingBuffer<T, N, CMP>::fill;
    };
    ...
    public:
        ...
        bool compare(X a, Y b)
        {
            return a == b;
        }
    };
    template <class Key, class T, typename CMP = _Compare, size_t N = ARX_MAP_DEFAULT_SIZE>
    struct map : public RingBuffer<pair<Key, T>, N, CMP>
    {
        using base = RingBuffer<pair<Key, T>, N, CMP>;
        ...
        const_iterator find(const Key &key) const
        {
            ...
            {
                if (compare.compare(key, it->first))
                {
                    return it;
                }
            }
...
template <typename T, size_t N, typename CMP>
using ArxRingBuffer = arx::RingBuffer<T, N, CMP>;
